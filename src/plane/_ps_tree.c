...
//  std::cout << "lrot\n";
//  std::cout << "p :" << p->split_value_x() << "\n";
//  std::cout << "q :" << q->split_value_x() << "\n";
  ps_item h = p->sohn[right];
//  std::cout << "h :" << h->split_value_x() << "\n";
  xh = h->x_value();
  yh = h->y_value();
...
ps_item ps_tree::sink(ps_item q, x_typ x, x_typ y)
{
 ...
 p->y_val=y;
 if (cmp(p->x_value(),xneu)==0 && cmp(p->y_value(),yneu)==0) inserted=p;
 // std::cout << "inserted" << inserted->split_value_y() <<":"<< inserted->x_value() << ":" << inserted->y_value() <<"\n";
 return inserted;
}
 ...
 if (cmp(y0,p->y_value())>=0 && p!=0)
 {
  if (cmp(x1,p->split_value_x())<=0 && cmp(p->split_value_x(),x2)<=0)
  {
   if (cmp(x1,p->x_value())<=0 && cmp(p->x_value(),x2)<=0)
      std::cout <<p->split_value_x()<<"("<<p->x_value()<<","<<p->y_value()<<")\n";
...
void ps_tree::pr_ps_tree(ps_item p,int blancs)
{
 if (p==0)
   ...
   { pr_ps_tree(p->sohn[left],blancs+10);
     for (int j=1;j<=blancs;j++) std::cout << " ";
     std::cout << "(" << p->split_value_x() << "," << p->split_value_y() << "):";
     std::cout << "(" << p->x_value() << "," << p->y_value() << ")\n";
