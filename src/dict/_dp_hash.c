...
{ unsigned long dp_k1=k>>16;\
  ...
  if (dp_f<0)\
    dp_erg=(unsigned long)(dp_f+2147483659);\
  else  dp_erg=(unsigned long)(dp_f);\
}
...
static void dpmod(unsigned long k, GenPtr x, unsigned long& dp_erg)
DPMOD_BODY(k,x,dp_erg)
