...
class SWEEP_POINT
{
  ...
  friend double    get_x(SWEEP_point p);
  friend double    get_y(SWEEP_point p);
  friend int       get_kind(SWEEP_point p);
  friend SWEEP_segment get_seg(SWEEP_point p);
  ...
  inline double    get_x(SWEEP_point p)       { return p->x; }
  inline double    get_y(SWEEP_point p)       { return p->y; }
  inline int       get_kind(SWEEP_point p)    { return p->kind; }
  inline SWEEP_segment get_seg(SWEEP_point p) { return p->seg; }


#if defined(__GNUC__)
// if I declare compare static g++ does not use it but instantiates
// the default error compare template
inline
#else
static
#endif
int compare(const SWEEP_point& p1, const SWEEP_point& p2)
{ if (p1==p2) return 0;
...
void Print(SWEEP_point& p) { std::cout << string("(%f,%f)",get_x(p), get_y(p)); }
...
class SWEEP_SEGMENT
{
 ...
 ~SWEEP_SEGMENT() { delete startpoint; delete endpoint; }
  ...
  friend SWEEP_point get_startpoint(SWEEP_segment seg);
  ...
  friend int  get_color(SWEEP_segment seg);
...
inline SWEEP_point get_startpoint(SWEEP_segment seg) { return seg->startpoint; }
inline SWEEP_point get_endpoint(SWEEP_segment seg)   { return seg->endpoint; }
inline double get_slope(SWEEP_segment seg)           { return seg->slope; }
inline double get_yshift(SWEEP_segment seg)          { return seg->yshift; }
inline int  get_orient(SWEEP_segment seg)            { return seg->orient; }
inline int  get_color(SWEEP_segment seg)             { return seg->color; }
inline int  get_name(SWEEP_segment seg)              { return seg->name; }
...
void Print(SWEEP_segment& s) { std::cout << string("S%d",get_name(s)); }
